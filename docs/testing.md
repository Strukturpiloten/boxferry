...
sensitive-document debug redaction. Compose import tests cover mapping and sequence labels,
key-only and scalar values, and name/value provenance across merged files.
...
`AddHost`, protected service labels, systemd quoting and literal `%` escaping, and
