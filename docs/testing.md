...
redaction, explicit compatible pod grouping, network/port conflict rejection, partial-candidate
authorization, target coverage boundaries, and omitted-maximum reporting.
...
provenance, native graph completeness, and strict-versus-partial policy behavior.
...
the complete native dependency graph, retained source provenance, and the required
`AllowApproximate` authorization.
...
feature boundaries independently. CI also runs markdownlint and lychee over the documentation.
