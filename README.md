...
exporter uses QuadletLens 0.1.11 for typed native construction and capability evidence, supports
...
ComposeLens 0.1.15 service `env_file` declarations enter the neutral graph without opening the
...
repeatable Quadlet `Label=` entries through ComposeLens 0.1.15 and QuadletLens 0.1.11. Empty and
...
ComposeLens 0.1.15 service dependencies retain source order, short/long defaults, and field-level
...
ComposeLens 0.1.15 service `restart` values now map into the distinct neutral container restart
...
QuadletLens 0.1.11. Separate-container output maps numeric primary GIDs and named or numeric
...
`ProtectedString` boundary as environment values. ComposeLens 0.1.15 imports these definitions and
grants. QuadletLens 0.1.11 emits exact mounted-file `Secret=` references for pre-existing external
